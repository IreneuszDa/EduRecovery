...
import React from 'react'; // Dodano import React dla JSX
                ...
                <div className="xl:grid xl:grid-cols-5 xl:gap-8">
                    ...
                    <div className="space-y-6 xl:col-span-2">
                        <a href="/" className="flex items-center space-x-3">
                            <Image src="/logo1t.png" alt="Uczmy.pl Logo" width={40} height={40} className="h-10 w-auto" />
...
export default function FormularzSzkola() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!data.name || !data.email || !data.school || !data.location || !data.students || !data.message) {
    ...
    try {
      const response = await fetch('/api/school-submission', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(responseData.message || 'Wystąpił nieoczekiwany błąd.');
      }
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-slate-900 text-gray-900 dark:text-gray-100 scroll-smooth flex flex-col">
      <div className="flex-grow">
        ...
        {/* Zintegrowany, nowy Header */}
        <Header isScrolled={isScrolled} />

        <section className="pt-32 pb-20 px-4">
          <h3 className="text-xl text-center md:text-5xl font-bold mb-6">
            Zgłoszenie szkoły do współpracy
          </h3>
          <p className="text-center text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mb-12">
            Wypełnij poniższy formularz, aby zgłosić szkołę do programu współpracy. Po otrzymaniu zgłoszenia skontaktujemy się z Państwem w ciągu 48 godzin.
          </p>

          <form
            onSubmit={handleSubmit}
            className="max-w-2xl mx-auto space-y-6 bg-gray-50 dark:bg-slate-800 p-8 rounded-xl shadow-lg"
          >
            <div>
              <label className="block mb-1 font-medium text-gray-800 dark:text-gray-200">Imię i nazwisko osoby kontaktowej</label>
              <input type="text" name="name" required className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100" />
            </div>
            <div>
              <label className="block mb-1 font-medium text-gray-800 dark:text-gray-200">Adres e-mail</label>
              <input type="email" name="email" required className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100" />
            </div>
            <div>
              <label className="block mb-1 font-medium text-gray-800 dark:text-gray-200">Nazwa szkoły</label>
              <input type="text" name="school" required className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100" />
            </div>
            <div>
              <label className="block mb-1 font-medium text-gray-800 dark:text-gray-200">Miasto i województwo</label>
              <input type="text" name="location" required className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100" />
            </div>
            <div>
              <label className="block mb-1 font-medium text-gray-800 dark:text-gray-200">Liczba uczniów</label>
              <input type="number" name="students" min="1" required className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100" />
            </div>
            <div>
              <label className="block mb-1 font-medium text-gray-800 dark:text-gray-200">Wiadomość lub dodatkowe informacje</label>
              <textarea name="message" rows={5} required className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-gray-100"></textarea>
            </div>
            
            <button
              type="submit"
              disabled={loading}
              className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-3 rounded-full shadow-lg transition-all duration-300 transform hover:scale-105 w-full disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
      ...
      {/* Nowy, zintegrowany komponent Footer */}
      <Footer />
    </main>
  );
}